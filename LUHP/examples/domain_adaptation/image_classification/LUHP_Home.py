...
def data_load(args):
    ...
    if args.source == ['A']:
        s_dset_path = "D:\\data\\office_home\\Art.txt"
    elif args.source == ['C']:
        s_dset_path = "D:\\data\\office_home\\Clipart.txt"
    elif args.source == ['P']:
        s_dset_path = "D:\\data\\office_home\\Product.txt"
    else:
        s_dset_path = "D:\\data\\office_home\\Real_World.txt"

    if args.target == ['A']:
        t_dset_path = "D:\\data\\office_home\\Art.txt"
    elif args.target == ['C']:
        t_dset_path = "D:\\data\\office_home\\Clipart.txt"
    elif args.target == ['P']:
        t_dset_path = "D:\\data\\office_home\\Product.txt"
    else:
        t_dset_path = "D:\\data\\office_home\\Real_World.txt"
    print(s_dset_path)
    print(t_dset_path)
...
def main(args: argparse.Namespace):
    ...
    mem_cls_target = torch.rand(len(dset_loaders["target"].dataset), 65).to(device)
    ...
    # start training
    best_acc1 = 0.
    luhp = LUHP.MemoryTripletK_reuse(65, len(dset_loaders["target"].dataset))
    for epoch in range(args.epochs):
        ...
        mem_fea_target, mem_cls_target = train(dset_loaders,  train_source_iter, train_target_iter, classifier, domain_adv, optimizer,
              lr_scheduler, epoch, args, mem_fea_target, mem_cls_target, luhp)
...
def train(dset_loaders, train_source_iter, train_target_iter,
          ...
          lr_scheduler: LambdaLR, epoch: int, args: argparse.Namespace, mem_fea_target, mem_cls_target, luhp):

    losses = AverageMeter('Loss', ':6.2f')
    cls_accs = AverageMeter('Cls Acc', ':6.1f')
    tgt_accs = AverageMeter('Tgt Acc', ':6.1f')
    ...
    for i in range(args.iters_per_epoch):
        ...
        src = CrossEntropyLabelSmooth(num_classes=65, epsilon=args.smooth)(y_s, labels_s)
